...
class Chat extends Component
{
    ...
    public function appendNewMessage($event)
    {
        ...
        if ($event['message']['conversation_id'] == $this->conversation->id) {
            ...
            $newMessage = WirechatService::messageModelClass()::find($event['message']['id']);
            // dd($newMessage);
    ...
    public function setReply(string $id): void
    {
        ...
        $message = WirechatService::messageModelClass()::where('id', $messageId)->firstOrFail();
    ...
    private function initializeConversation($conversation)
    {
        ...
        if ($conversation instanceof Conversation) {
            ...
            $this->conversation = WirechatService::conversationModelClass()::find($conversationId);
        ...
        // $this->conversation = WirechatService::conversationModelClass()::where('id', $conversation)->firstOr(fn () => abort(404));
    ...
    private function initializeParticipants()
    {
        if (in_array($this->conversation->type, [ConversationType::PRIVATE, ConversationType::SELF])) {
            ...
            $this->authParticipant = WirechatService::participantModelClass()::where('conversation_id', $this->conversation->id)->whereParticipantable($this->sendable)->first();
            $this->receiver = null;
        }
    }
